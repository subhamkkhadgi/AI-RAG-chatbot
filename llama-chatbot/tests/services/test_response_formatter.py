...
class TestRomanNumeralLists:
    """Verify conservative handling of malformed inline Roman numeral lists."""

    def test_malformed_roman_inline_list_fixed(self) -> None:
        """An inline Roman list without line breaks is split onto lines."""
        text = "I. Objective one. II. Objective two. III. Objective three."
        assert (
            strip_attribution(text)
            == "I. Objective one.\nII. Objective two.\nIII. Objective three."
        )

    def test_valid_roman_list_unchanged(self) -> None:
        """A properly formatted Roman list is preserved."""
        text = "I. Objective one.\nII. Objective two.\nIII. Objective three."
        assert strip_attribution(text) == text

    def test_chapter_version_section_unchanged(self) -> None:
        """Chapter I / Version II / Section III are not treated as lists."""
        text = "See Chapter I. Version II is released. Section III covers setup."
        assert strip_attribution(text) == text

    def test_roman_markers_in_code_block_unchanged(self) -> None:
        """Roman-markers inside a code block are never modified."""
        text = "```\nI. Objective one. II. Objective two.\n```"
        assert strip_attribution(text) == text

    def test_mixed_arabic_and_roman_list_unchanged(self) -> None:
        """Arabic broken markers are still fixed; valid Roman markers are
        preserved on their own lines."""
        text = "1. Feature one2. Feature two\nI. Objective one.\nII. Objective two."
        expected = "1. Feature one\n2. Feature two\nI. Objective one.\nII. Objective two."
        assert strip_attribution(text) == expected

    def test_single_roman_marker_unaffected(self) -> None:
        """A single Roman marker is not treated as a list."""
        text = "I. Objective one."
        assert strip_attribution(text) == text

    def test_roman_markers_not_following_period_unchanged(self) -> None:
        """Roman markers that follow a word (not a period) are not split."""
        text = "The I. and II. phases completed."
        assert strip_attribution(text) == text
